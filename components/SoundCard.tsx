  ...
  return (
    <div className={`group relative p-6 rounded-2xl border transition-all duration-300 ${isPlaying ? getGlow() + ' bg-zinc-900' : 'bg-zinc-900/50 border-zinc-800 hover:bg-zinc-900'}`}>
      ...
      <div className="flex justify-between items-start mb-3">
        ...
        <button
          onClick={onToggle}
          aria-label={isPlaying ? `Stop playing ${data.title}` : `Play ${data.title}`}
        ...
        >
          {isPlaying ? (
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><rect x="6" y="4" width="4" height="16"></rect><rect x="14" y="4" width="4" height="16"></rect></svg>
          ) : (
            <svg className="w-4 h-4 ml-1" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5v14l11-7z"></path></svg>
          )}
        </button>
      </div>
      ...
      <div className={`overflow-hidden transition-all duration-300 ${isPlaying ? 'max-h-24 opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
        <div className="flex items-center gap-3">
           ...
           <input
             ...
             aria-label={`Volume for ${data.title}`}
